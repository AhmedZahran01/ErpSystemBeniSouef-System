using ErpSystemBeniSouef.Core.DTOs.CustomerInvoiceDtos.PayAnIncrease;

namespace ErpSystemBeniSouef.Service.CustomerInvoiceServices;

public class CustomerInvoiceService(IUnitOfWork unitOfWork) : ICustomerInvoiceService
{
    ...
    #region Pay an Increase

    public async Task<ServiceResponse<List<MonthlyInstallmentDto>>> GetMonthlyInstallmentsByInvoiceIdAsync(int customerInvoiceId)
    {
        var query = _unitOfWork.Repository<MonthlyInstallment>()
            .GetAllQueryable(x => x.Customer, x => x.Customer.SubArea!);

        var installments = await query
            .Where(x => x.InvoiceId == customerInvoiceId)
            .OrderBy(x => x.MonthDate)
            .Select(x => new MonthlyInstallmentDto
            {
                Id = x.Id,
        ...
        return ServiceResponse<List<MonthlyInstallmentDto>>
            .SuccessResponse(installments, "Monthly installments retrieved successfully.");
    }

    public async Task<ServiceResponse<bool>> UpdateInstallmentsAsync(UpdateMonthlyInstallmentsDto dto)
    {
        ...
        {
            var updated = dto.Installments
                .FirstOrDefault(x => x.Id == installment.Id);

            if (updated is null)
                continue;

            installment.Amount = updated.Amount;
        }

        await _unitOfWork.CompleteAsync();

        return ServiceResponse<bool>
            .SuccessResponse(true, "Installments updated successfully.");
    }
